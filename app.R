        ...
        shiny::HTML(
          paste(
            c(
              ...
              "  }",
              "  function updateSidebarVisibility(nextTabName) {",
              "    var tab = nextTabName || activeTabName();",
              "    var hide = tab === 'Inequality & Overlap' || tab === 'Model Lab' || tab === 'Data & Methods';",
              "    var showBusyBar = tab === 'Inequality & Overlap' || tab === 'Model Lab';",
              "    if (document.body) document.body.classList.toggle('hide-global-sidebar', hide);",
              "    if (document.body) document.body.classList.toggle('show-busy-bar', showBusyBar);",
              ...
              "    // Track global Shiny busy/idle state so we can show a top loading bar on heavy tabs.",
              "    if (window.jQuery) {",
              "      window.jQuery(document).on('shiny:busy', function() {",
              "        if (document.body) document.body.classList.add('shiny-is-busy');",
              "      });",
              "      window.jQuery(document).on('shiny:idle', function() {",
              "        if (document.body) document.body.classList.remove('shiny-is-busy');",
              "      });",
              "    }",
              "    // Update ASAP on tab switches to avoid a 'sidebar flash' on tabs that hide it.",
              "    document.addEventListener('show.bs.tab', function(e) {",
              "      var next = e && e.target ? e.target.getAttribute('data-value') : null;",
              "      updateSidebarVisibility(next);",
              "    });",
              "    document.addEventListener('shown.bs.tab', function() { updateSidebarVisibility(); });",
